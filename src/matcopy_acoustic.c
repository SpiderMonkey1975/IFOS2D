...
void matcopy_acoustic( float ** rho, float ** pi ) {
	...
	int i, n;
        ...
        send_buf = (float *)malloc( 4*NX*sizeof(float) );
        recv_buf = (float *)malloc( 4*NX*sizeof(float) );
    ...
    /* storage of bottom of local volume into buffer */
        ...
        MPI_Isend( send_buf, 4*NX, MPI_FLOAT, INDEX[3], TAG5, MPI_COMM_WORLD, &req[0] );
        MPI_Irecv( recv_buf, 4*NX, MPI_FLOAT, INDEX[4], TAG5, MPI_COMM_WORLD, &req[1] );
        ...
        send_buf = (float *)realloc( send_buf, 4*(NY+2)*sizeof(float) );
        recv_buf = (float *)realloc( recv_buf, 4*(NY+2)*sizeof(float) );
     ...
     /* storage of right edge of local volume into buffer */
        ...
        MPI_Isend( send_buf, 4*(NY+2), MPI_FLOAT, INDEX[1], TAG1, MPI_COMM_WORLD, &req[0] );
        MPI_Irecv( recv_buf, 4*(NY+2), MPI_FLOAT, INDEX[2], TAG1, MPI_COMM_WORLD, &req[0] );
        MPI_Waitall( 2, req, stat );
